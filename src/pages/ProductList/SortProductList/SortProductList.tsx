...
const SortProductList = ({ queryConfig, pageSize }: SortProductListProps) => {
  ...
  const handleSortByPrice = (sortByValue: Exclude<ProductListConfig['order'], undefined>) => {
    navigate({
      pathname: path.home,
      search: createSearchParams({
        ...queryConfig,
          ...
          <div className='flex items-center flex-wrap gap-2'>
            ...
            <button
              className={classNames('h-8 px-4 capitalize text-sm text-center', {
                'bg-orange hover:bg-orange/80 text-white': isActive('createdAt'),
                'bg-white hover:bg-slate-100 text-black': !isActive('createdAt')
              })}
            ...
            <select
              className={classNames('h-8 px-4 capitalize text-black text-sm text-left outline-none', {
                'bg-orange hover:bg-orange/80 text-white': isActive('price'),
                'bg-white hover:bg-slate-100 text-black': !isActive('price')
              })}
              value={order || ''}
              onChange={(e) => handleSortByPrice(e.target.value as Exclude<ProductListConfig['order'], undefined>)}
            >
